...
class KKContentExt(KKBaseExtractor):
    def __init__(self,html):
        super(KKContentExt,self).__init__(html)
    
    def parse(self):
        lis = self.doc('#article_eng>div>div')
        content = []
        for i in range(0,len(lis)):
            pq_li = pq(lis[i])
            content.append(pq_li.text())
        return content
...
for link in psg_links:
    ...
    conExt = KKContentExt(html)
    content = conExt.parse()
    psg_link = link[1]
    psg_title = link[0]
    temp_dict = { 'title': psg_title, 'link': psg_link, 'content': content}
    total_content.append(temp_dict)

# 将爬取内容写入文件中
with open('20191120week11练习1输出文本.txt', "w",encoding='utf-8') as f:
